...
//    fatal("WebM output to pipes not supported.");
  ...
  if (stream->config.write_webm) {
    ...
    int chunk_size = 0;
    while (is_chunk_ready(&stream->ebml, &chunk_size)) {
      cluster_page_t * page = streamer_get_free_allocation(webm_cluster_table);
      page->length = chunk_size;
      page->webm_cluster = malloc(chunk_size);

      read_chunk(&stream->ebml, page->length, page->webm_cluster);
      stream_allocation_ready(webm_cluster_table, page);

      /*    unsigned char* buffer;
      buffer = malloc(chunk_size);

      int ret = read_chunk(&stream->ebml, chunk_size, buffer);
      if (ret) {
        printf("read_chunk %d\n", ret);
        exit(-1);
      }

      fwrite(buffer, 1, chunk_size, stream->file);
      free(buffer);*/
    }
  }
#endif
...
static void encode_frame(struct stream_state *stream,
  ...
  next_frame_start = (cfg->g_timebase.den * (int64_t)(frames_in+1)
                      * global->framerate.den)
                     / cfg->g_timebase.num / global->framerate.num;
        ...
        if (stream->config.write_webm) {
          ...
          while (is_chunk_ready(&stream->ebml, &chunk_size)) {
           ...
           /* unsigned char* buffer;
            ...
            read_chunk(&stream->ebml, chunk_size, buffer);
            fwrite(buffer, 1, chunk_size, stream->file);
            free(buffer);*/
          }
        }
